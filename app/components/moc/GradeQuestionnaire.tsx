...
export function GradeQuestionnaire({ workType, questionList, onComplete, onTemporarySave, onBack, initialAnswers, draftKey }: { workType: WorkType; contextText: string; questionList: Question[]; onComplete: (result: GradeRecommendation, answers: Record<string, RuleAnswer>) => void; onTemporarySave: (answers: Record<string, RuleAnswer>) => void; onBack: () => void; initialAnswers?: Record<string, RuleAnswer>; draftKey?: string }) {
  ...
  const availableRules = useMemo(() => gradeRules
    .filter((rule) => questionList.some((question) => question.id === `grade:${rule.id}`))
    .sort((a, b) => (questionList.find((question) => question.id === `grade:${a.id}`)?.order ?? 0) - (questionList.find((question) => question.id === `grade:${b.id}`)?.order ?? 0)), [questionList]);
  const selectedRuleIds = useMemo(() => Object.entries(answers).filter(([, answer]) => answer === "YES").map(([id]) => id).filter((id) => availableRules.some((rule) => rule.id === id)), [answers, availableRules]);
  const selectedGrades = useMemo(() => availableRules.filter((rule) => selectedRuleIds.includes(rule.id)).map((rule) => rule.grade), [availableRules, selectedRuleIds]);
  ...
  return <div className="focused-page wide grade-table-page"><div className="step-head"><button className="back-link" onClick={onBack}>← 변경판정</button><div><span>변경관리 등급판정</span><b>붙임 3</b></div><div className="progress-track"><i style={{ width: "100%" }}/></div></div>
    <section className="question-card grade-table-card"><div className="question-meta"><span className="security-chip">붙임 3 · 변경관리등급 기준</span><span>{workType}</span></div><h1>해당 변경 항목을 모두 선택해 주세요.</h1><p className="question-help"><span>i</span>질문을 순서대로 답하지 않습니다. 표에서 해당되는 항목을 여러 개 선택하면, 숫자가 가장 높은 등급(1등급이 최상위)으로 최종 판정합니다. 예: 2등급과 3등급을 함께 선택하면 2등급입니다.</p>
      ...
      <div className="grade-table-wrap"><table><thead><tr><th>구분</th><th>대상항목</th><th>해당</th></tr></thead><tbody>{availableRules.map((rule) => <tr key={rule.id}><td>{rule.category}</td><td>{questionList.find((question) => question.id === `grade:${rule.id}`)?.text ?? rule.title}</td><td><label className="grade-check"><input type="checkbox" checked={answers[rule.id] === "YES"} onChange={(event) => toggleRule(rule.id, event.target.checked)}/><span>선택</span></label></td></tr>)}</tbody></table></div>
