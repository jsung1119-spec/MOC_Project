...
export function NewMocCaseForm({ onSubmit, onTemporarySave, onBack, initialInfo, initialAssetType, draftKey }: { onSubmit: (info: MocBasicInfo, assetType: AssetType) => void; onTemporarySave: (info: MocBasicInfo, assetType: AssetType) => void; onBack: () => void; initialInfo?: MocBasicInfo; initialAssetType?: AssetType; draftKey?: string }) {
  ...
  const [info, setInfo] = useState(() => ({ ...(initialInfo ?? empty), writtenDate: today }));
  const [assetType, setAssetType] = useState<AssetType>(initialAssetType ?? "PUMP");
  const [attempted, setAttempted] = useState(false);
  const completionMinimumDate = today;
  useEffect(() => {
    setInfo({ ...(initialInfo ?? empty), writtenDate: today });
    setAssetType(initialAssetType ?? "PUMP");
    setAttempted(false);
  }, [draftKey, initialInfo, initialAssetType, today]);
  ...
  return <div className="focused-page wide guideline-form"><div className="step-head"><button className="back-link" onClick={onBack}>← 대시보드</button><div><span>변경사항 입력</span><b>1 / 4</b></div><div className="progress-track"><i style={{ width: "25%" }}/></div></div>
    <section className="question-card start-card"><span className="eyebrow">STEP 01 · 변경 발의</span><h1>변경사항을 먼저 알려주세요</h1><p>변경판정과 등급판정에 필요한 기본정보입니다. 입력 내용은 이후 변경 요청/승인서에도 사용됩니다.</p>
      <div className="form-grid basic-info-grid">
        ...
        <GuidelineField label="작성일자" value={today} readOnly onChange={() => undefined}/>
      ...
      <div className="question-footer"><button className="btn ghost" onClick={onBack}>← 이전</button><button type="button" className="btn soft" onClick={() => onTemporarySave(info, assetType)}>임시저장</button><button className="btn primary" onClick={submit}>변경판정 시작 →</button></div>
    </section></div>;
}
...
function GuidelineField({ label, value, onChange, error, area, type = "text", min, readOnly, className = "" }: { label: string; value: string; onChange: (value: string) => void; error?: string; area?: boolean; type?: string; min?: string; readOnly?: boolean; className?: string }) {
  return <label className={`field ${className} ${error ? "missing" : ""}`}><span>{label}<em>*</em></span>{area ? <textarea value={value} onChange={(event) => onChange(event.target.value)}/> : <input type={type} value={value} onInput={(event) => onChange(event.currentTarget.value)} min={min} readOnly={readOnly}/>} {error && <small className="field-error">{error}</small>}</label>;
}
