...
export default function Home() {
  ...
  const main = view === "dashboard" ? <Dashboard cases={siteCases} reminders={reminders} onNew={() => { setActiveId(""); go("new"); }} onOpen={continueCase} onRename={renameCase} onReminder={() => go("reminders")} onHistory={(chartFilter) => { setFilter(chartFilter ?? ""); go("history"); }} />
...
function Dashboard({ cases, reminders, onNew, onOpen, onRename, onReminder, onHistory }: { cases: MocCase[]; reminders: MocCase[]; onNew: () => void; onOpen: (c: MocCase) => void; onRename: (id: string, title: string) => void; onReminder: () => void; onHistory: (chartFilter?: string) => void }) {
